...
class QuantumStateBuilder:
    def __init__(self,tag):
        self.tag = tag #this contains the information to build the array representing psi0
        new_tag = self.tag.copy()
        new_tag.remove('_') #Now the list must have an even number of elements.
        self.system_size = len(new_tag)/2
        self.directions = new_tag[0:self.system_size]
        self.excitations = new_tag[self.system_size+1:]
        assert len(self.excitations)==len(self.directions), 'The read tag has non-matching sides: {val1} and {val2}'.format(val1=self.excitations,val2=self.directions)

    def build_state(self):
        psi0 = 1
        for k in range(self.system_size):
            psi0 = np.kron(psi0,QQ.pauli_states(self.directions[k],self.excitations[k]))
        return QQ.regularize_state(psi0)
